...
{
	// Forward hwnd on because we can get messages (e.g., WM_CREATE) before CreateWindow returns, and thus before mhMainWnd is valid.
	return GraphicEngine::GetApp()->MsgProc(hwnd, msg, wParam, lParam);
}

GraphicEngine* GraphicEngine::mApp = nullptr;
GraphicEngine* GraphicEngine::GetApp()
{
	return mApp;
}

GraphicEngine::GraphicEngine(HINSTANCE hInstance) : mhAppInst(hInstance)
{
	// Only one GraphicEngine can be constructed.
	assert(mApp == nullptr);
	mApp = this;
}
...
HINSTANCE GraphicEngine::AppInst()const
{
	return mhAppInst;
}
...
bool GraphicEngine::Get4xMsaaState()const
{
	return m4xMsaaState;
}

void GraphicEngine::Set4xMsaaState(bool value)
{
...
int GraphicEngine::Run()
{
	...
	{
		// If there are Window messages then process them, otherwise do game stuff
...
LRESULT GraphicEngine::MsgProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	switch( msg )
	{
...
bool GraphicEngine::InitMainWindow()
{
	WNDCLASS wc;
...
bool GraphicEngine::InitDirect3D()
{
	...
	// All Direct3D 12 capable devices support 4X MSAA for all render target formats, so we only need to check quality support.
...
void GraphicEngine::CreateSwapChain()
{
	...
	// For Direct3D 12 the first param is a pointer to a direct command queue.
...
void GraphicEngine::CalculateFrameStats()
{
	...
	if ((m_timer.TotTime() - timeElapsed) >= 1.0f)
	{
		float fps = (float)frameCnt; // fps = frameCnt / 1

		wstring fpsStr = to_wstring(fps);

		wstring windowText = mMainWndCaption + L"    fps: " + fpsStr;
